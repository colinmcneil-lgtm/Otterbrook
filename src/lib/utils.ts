import { CandidateStage, MandateStatus } from "@prisma/client";
...
export function formatStatus(status: MandateStatus): string {
  const map: Record<MandateStatus, string> = {
    OPEN: "Open",
    CLOSED: "Closed",
    ON_HOLD: "On Hold",
  };
...
export function statusColor(status: MandateStatus): string {
  const map: Record<MandateStatus, string> = {
